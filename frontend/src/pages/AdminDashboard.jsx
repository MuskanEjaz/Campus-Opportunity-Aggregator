    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            {stats && (
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
                    <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-5 text-center">
                        <p className="text-3xl font-bold text-indigo-600">{stats.total_active}</p>
                        <p className="text-sm text-gray-500 mt-1">Active Opportunities</p>
                    </div>
                    <div className="bg-red-50 border border-red-100 rounded-xl p-5 text-center">
                        <p className="text-3xl font-bold text-red-500">{stats.total_expired}</p>
                    ...
                    <div className="bg-yellow-50 border border-yellow-100 rounded-xl p-5 text-center">
                        <p className="text-3xl font-bold text-yellow-600">{stats.total_saves}</p>
                        <p className="text-sm text-gray-500 mt-1">Total Saves</p>
                    </div>
                </div>
            )}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-red-100 p-6 mb-8">
                <h2 className="text-lg font-semibold text-red-600 mb-4">⏰ Expiring Soon (Next 3 Days)</h2>
                {expiring.length === 0 ? (
                    <p className="text-gray-400 text-sm">No opportunities expiring soon.</p>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {expiring.map(opp => (
                            <div key={opp.opp_id} className="border border-red-100 rounded-lg p-4 bg-red-50">
                                <p className="font-semibold text-gray-800">{opp.title}</p>
                                <p className="text-xs text-gray-500 mt-1">{opp.category} · {opp.department}</p>
                                <p className="text-xs text-red-600 font-semibold mt-2">{getDaysLeft(opp.deadline)}</p>
                            </div>
                        ))}
                    </div>
                )}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-indigo-100 p-6 mb-8">
                <h2 className="text-lg font-semibold text-indigo-600 mb-4">⭐ Recommended For You</h2>
                {recommendations.length === 0 ? (
                    <p className="text-gray-400 text-sm">No recommendations available.</p>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {recommendations.map(opp => (
                            <div key={opp.opp_id} className="border border-indigo-100 rounded-lg p-4 bg-indigo-50">
                                <p className="font-semibold text-gray-800">{opp.title}</p>
                                <p className="text-xs text-gray-500 mt-1">{opp.category} · {opp.department}</p>
                                <p className="text-xs text-indigo-600 font-semibold mt-2">Score: {opp.trend_score}</p>
                            </div>
                        ))}
                    </div>
                )}
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {opportunities.map(opp => (
                                <tr key={opp.opp_id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4">
                                        <button onClick={() => handleDelete(opp.opp_id)} className="text-red-500 hover:text-red-700 text-xs font-semibold">
                                            Delete
                                        </button>
                                    </td>
                                </tr>
